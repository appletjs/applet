...
上下文分为 **Applet应用上下文** 和 **handle函数执行上下文**。

每个中间件都接收一个纯对象 `Object`，正常情况下，该对象初始化状态仅仅包含了 Applet 实例的下面两个简单配置为属性。
`ctx` 通常用作上下文对象的参数名称。

* **env** - 运行环境，默认值为 `'development'`，默认由环境变量定义，或者通过实例指定：
  ```js
  // 指定 Node.js 的环境变量
...
> 正常情况下，在执行 **handle函数** 时中修改 Applet 应用的 `slient`、`env` 属性，
是无法作用到handle函数执行上下文，除非通过自定义的handle函数执行上下文建立响应。
